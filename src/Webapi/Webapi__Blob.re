module Impl = (T: {type t}) => {
  ...
  [@mel.send.pipe: T.t] external slice: (~start: int=?, ~end_: int=?, ~contentType: string=?) => T.t =
    "slice";

  /** @since 0.19.0 */
  [@mel.send] external stream: T.t => Webapi__ReadableStream.t = "stream";
  ...
  [@deprecated "Use [type_] instead"]
  [@mel.get] external _type: T.t => string = "type";
};
