...
let _: Image.t = createImageDataFromImage(ctx, imageData);
let _: float = Image.width(imageData);
let _: float = Image.height(imageData);

let _: Image.t = getImageData(ctx, ~sx=0.0, ~sy=0.0, ~sw=0.0, ~sh=0.0);
